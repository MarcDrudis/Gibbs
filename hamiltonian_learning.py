...
def create_hamiltonian_lattice(num_sites: int,j_const:float,g_const:float) -> SparsePauliOp:
...
def sample_pauli_basis(
    state: Statevector, pauli_basis: list, num_samples: int, noise: float = 0.0
) -> dict[str, float]:
...
if __name__ == "__main__":
    # hamiltonian =1  * SparsePauliOp.from_list(
    #     [
    #         ("XXIII", 1.5),
    #         ("IZZII", -3.0),
    #         ("IIYYI", 7.0),
    ...
    N = 7
    hamiltonian = create_hamiltonian_lattice(N)
    ...
    sampled_pauli = sample_pauli_basis(thermal_state, sampling_basis, 1000, noise=0)
    ...
    # eigvals, eigvecs = eigsh(KTK, k=5, which="SA")
    ...
    print(
        np.linalg.norm(
            rec_vectors[0] - original_vector / np.linalg.norm(original_vector)
        )
    )
# %%
